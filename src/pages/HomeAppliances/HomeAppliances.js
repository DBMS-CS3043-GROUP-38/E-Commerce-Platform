...
const HomeAppliances = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost:3000/api/products/Type/Home Appliances');
                if (!response.ok) {
    ...
    const handleAddToCart = (product) => {
        const imagePath = `/images/home-appliances/${product.Name}.png`;
        ...
        <div className="product-page">
            ...
            <div className="product-grid">
                {products.length > 0 ? (
                    ...
                    <p>No home appliances products available.</p>
                )}
            </div>
        </div>
    );
